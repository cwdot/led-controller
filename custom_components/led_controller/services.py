...
def async_register_services(hass: HomeAssistant) -> None:
    ...
    async def _set_led(call: ServiceCall) -> None:
        ...
        for coord in coordinators:
            ...
            written = LedState(
                on=True, color=color_for_device, brightness_pct=brightness, mode=mode
            )
            # When every LED gets the same color, prefer a single bulk write if the
            # device exposes one (Inovelli param-99 / led_effect) — one round trip
            # instead of N sequential ones.
            full_range = leds == list(range(1, coord.device.led_count + 1))
            bulk = getattr(coord.device, "set_all", None)
            if full_range and bulk is not None:
                await bulk(hass, color_for_device, brightness)
                for led_idx in leds:
                    coord.record_write(led_idx, written)
                continue
            for led_idx in leds:
...
def _expand_target_device_ids(hass: HomeAssistant, call: ServiceCall) -> set[str]:
    """Resolve device_id / area_id / entity_id targets to a set of device_ids."""
    device_ids: set[str] = set(_as_list(call.data.get("device_id")))

    area_ids = set(_as_list(call.data.get("area_id")))
    entity_ids = set(_as_list(call.data.get("entity_id")))
    if not area_ids and not entity_ids:
        return device_ids
