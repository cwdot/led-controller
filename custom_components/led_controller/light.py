...
class LedControllerLight(CoordinatorEntity[LedControllerCoordinator], LightEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        if ATTR_BRIGHTNESS in kwargs:
            ...
            # Preserve the current level on a color-only change; fall back to full.
            existing_pct = self._state.brightness_pct if self._state else None
            brightness_pct = existing_pct if existing_pct is not None else 100
