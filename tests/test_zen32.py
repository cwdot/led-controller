...
async def test_read_all_omits_led_on_read_failure():
    """A failed per-LED read should be omitted so the coordinator merge keeps prior state."""

    class _Services:
        async def async_call(self, domain, service, data, **kwargs):
            # LED 2's params are mode=3, color=8, brightness=13 — fail those reads.
            if data["parameter"] in (3, 8, 13):
                raise RuntimeError("z-wave timeout")
            return {"entity": {"param": {"value": 0}}}

    class _Hass:
        services = _Services()

    device = Zen32Device(device_id="dev-123", led_count=2)
    result = await device.read_all(_Hass())

    assert 1 in result  # healthy LED present
    assert 2 not in result  # failed LED omitted, not reset to a default state
