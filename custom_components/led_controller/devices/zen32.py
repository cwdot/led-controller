    ...
    async def read_all(self, hass: HomeAssistant) -> dict[int, LedState]:
        ...
        for led_idx in range(1, self.led_count + 1):
            try:
                ...
                # Omit the LED so the coordinator merge preserves its last-known state;
                # inserting a default LedState() here would flip the UI to "off" on a
                # transient z-wave timeout.
