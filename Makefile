...
INTEGRATION := custom_components/led_controller
...
$(DEV_CONFIG)/custom_components/led_controller: | $(DEV_CONFIG)/custom_components
	@ln -sfn $(CURDIR)/$(INTEGRATION) $@

run: install $(DEV_CONFIG)/custom_components/led_controller ## Launch a local HA with this integration symlinked
	$(HASS) -c $(DEV_CONFIG)
